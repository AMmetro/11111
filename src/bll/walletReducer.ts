...
// thunks-------------------------------------------------------------------
    export const withdrawalMoneyTC = (amount:number,currencyId:string) => {return (dispatch: Dispatch<ChangeWalletBalanceType>, getState: () => AppStoreType) => {
    ...
    export const enrolledMoneyTC = (amount:number,currencyId:string) => {return (dispatch: Dispatch<ChangeWalletBalanceType>, getState: () => AppStoreType) => {
        let currencyList = getState().currencyListReducer[0].list
        let currencyPrice = changeCurrency.getTradetCurrencyPrice(amount, currencyId, currencyList)
